...
window.addEventListener('load', () => {
    ...
    // --- Мобильное меню ---
    if (openMenu && hamburgerMenu && closeScene && bodyScroll) {
        openMenu.addEventListener('click', () => {
            bodyScroll.classList.add('scroll-no');
            hamburgerMenu.classList.add('active');
            closeScene.classList.add('active');
    ...
    /// 5. Консалтинг табы и фильтр
    const tabs = document.querySelectorAll('.consalting__tab');
    const consaltingItems = document.querySelectorAll('.consalting__item');
    const select = document.querySelector('.consalting__list-select');
    ...
    tabs.forEach((tab, index) => {
        tab.addEventListener('click', (e) => {
            tabs.forEach(t => t.classList.remove('active'));
            tab.classList.add('active');
            consaltingItems.forEach(item => item.classList.remove('active'));
            if (consaltingItems[index]) {
                consaltingItems[index].classList.add('active');
                const animatedItems = consaltingItems[index].querySelectorAll('.animate-on-scroll');
                animatedItems.forEach(el => {
                    if (!animatedFlags.has(el)) scrollObserver.observe(el);
                });
            }
    ...
    const initialIndex = Array.from(tabs).findIndex(t => t.classList.contains('active'));
    if (consaltingItems[initialIndex]) {
        consaltingItems[initialIndex].classList.add('active');
        const animatedItems = consaltingItems[initialIndex].querySelectorAll('.animate-on-scroll');
        animatedItems.forEach(el => {
            if (!animatedFlags.has(el)) scrollObserver.observe(el);
        });
    }
    ...
    if (wrapper && prevBtn && nextBtn && dotsContainer) {
        const sliderItems = wrapper.querySelectorAll('.packages-item');
        let currentIndex = 0;
...
}); // Закрываем window.addEventListener('load', ...)

// ============ ФОРМА (ВНЕ load, чтобы не ждать загрузки) ============
const form = document.querySelector('.contacts__form');
if (form) {
    const nameInput = form.querySelector('input[name="entry.2059123062"]');
    const emailInput = form.querySelector('input[name="entry.161505263"]');
    ...
    // Сообщения об ошибке
    const nameError = nameInput ? nameInput.parentElement.querySelector('.error-message') : null;
    const emailError = emailInput ? emailInput.parentElement.querySelector('.error-message') : null;
    const telegramError = telegramInput ? telegramInput.parentElement.querySelector('.error-message') : null;
    ...
    form.addEventListener('submit', (e) => {
        ...
        // Проверяем telegram
        if (telegramInput.value.trim() === "") {
            if (telegramError) telegramError.style.display = 'block';
            telegramInput.classList.remove('valid');
            telegramInput.classList.add('error');
            valid = false;
        } else {
            if (telegramError) telegramError.style.display = 'none';
            telegramInput.classList.remove('error');
            telegramInput.classList.add('valid');
        }
