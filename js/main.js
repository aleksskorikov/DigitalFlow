...
window.addEventListener('load', () => {
  ...
  if (localStorage.getItem('languageChanged') !== 'true') {
    ...
    // Пример для hero
    const mainDesc = document.querySelector('[data-elem-id="1642632663322"]');
    if (mainDesc) mainDesc.setAttribute('data-field-width-res-320-value', '261');
    ...
    // Можно аналогично остальные элементы — только с проверкой на null
  }

  // -----------------------------
  // 2. Header и мобильное меню
  // -----------------------------
  const bodyScroll = document.getElementById('bodyScroll');
  const hamburgerMenu = document.querySelector('.header__mobile-menu');
  const openMenu = document.querySelector('.header__hamburger');
  const close = document.querySelector('.header__close');
  const closeScene = document.querySelector('.header__scene');
  const menuLinks = document.querySelectorAll('.header__link-mobile');

  function closeMenu() {
    if (hamburgerMenu) hamburgerMenu.classList.remove('active');
    if (closeScene) closeScene.classList.remove('active');
    if (bodyScroll) bodyScroll.classList.remove('scroll-no');
  }

  if (openMenu) openMenu.addEventListener('click', () => {
    if (bodyScroll) bodyScroll.classList.add('scroll-no');
    if (hamburgerMenu) hamburgerMenu.classList.add('active');
    if (closeScene) closeScene.classList.add('active');
  });

  if (close) close.addEventListener('click', closeMenu);
  if (closeScene) closeScene.addEventListener('click', closeMenu);
  ...
  function updateLabelColors() {
    labels.forEach(label => {
      ...
      if (radio) label.style.color = radio.checked ? 'white' : '';
    });
  }

  radios.forEach(r => r.checked = r.value === language);
  updateBackgrounds(language);
  updateLabelColors();
  ...
  document.querySelectorAll('.animate-slide-in, .animate-on-scroll').forEach(el => scrollObserver.observe(el));
  ...
  if (form) {
    ...
    function isValidName(value) { return /^[A-Za-zА-ЩЬЮЯЄІЇҐа-щьюяєіїґ' -]+$/.test(value.trim()); }
    ...
    form.addEventListener('submit', (e) => {
      ...
      if (nameInput && nameError) {
        if (!isValidName(nameInput.value)) { valid = false; nameInput.classList.add('error'); nameError.style.display='block'; } 
        else { nameInput.classList.remove('error'); nameInput.classList.add('valid'); nameError.style.display='none'; }
      }

      if (emailInput && emailError) {
        if (!isValidEmail(emailInput.value)) { valid = false; emailInput.classList.add('error'); emailError.style.display='block'; } 
        else { emailInput.classList.remove('error'); emailInput.classList.add('valid'); emailError.style.display='none'; }
      }

      if (telegramInput && telegramError) {
        if (telegramInput.value.trim() === "") { valid = false; telegramInput.classList.add('error'); telegramError.style.display='block'; } 
        else { telegramInput.classList.remove('error'); telegramInput.classList.add('valid'); telegramError.style.display='none'; }
      }
      ...
      fetch(form.action, { method:'POST', body:formData, mode:'no-cors' })
        .then(() => {
          ...
          form.reset();
          [nameInput,emailInput,telegramInput].forEach(input=>input?.classList.remove('valid'));
